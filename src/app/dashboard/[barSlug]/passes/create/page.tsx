...
export default function CreatePassPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      setError(d.error || "Failed to create pass");
    }
    setSaving(false);
  };
    ...
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "14px", maxWidth: "560px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
        <div>
          ...
          <input type="number" step="0.01" style={inputStyle} value={price} onChange={(e) => setPrice(e.target.value)} placeholder="15.00" required />
        </div>
        <div>
          <label style={labelStyle}>Original Price (EUR)</label>
          <input type="number" step="0.01" style={inputStyle} value={originalPrice} onChange={(e) => setOriginalPrice(e.target.value)} placeholder="25.00" />
        </div>
      </div>

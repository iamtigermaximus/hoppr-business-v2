...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else setError("Admin access only. Use bar manager login.");
    setLoading(false);
  };
  ...
  return (
    <div style={{ minHeight: "100dvh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px", background: "var(--color-bg, #0a0a0a)" }}>
      <div style={{ background: "var(--color-card, #1a1a1a)", border: "1px solid var(--color-card-border, #262626)", borderRadius: "16px", padding: "32px", maxWidth: "400px", width: "100%" }}>
        <h1 style={{ fontWeight: 800, fontSize: "24px", color: "var(--color-text-primary, #fff)", marginBottom: "4px", textAlign: "center" }}>Hoppr Admin</h1>
        <p style={{ color: "var(--color-text-muted, #737373)", fontSize: "13px", textAlign: "center", marginBottom: "24px" }}>Platform management</p>
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          ...
          <button type="submit" disabled={loading} style={{ width: "100%", padding: "12px", background: "#7c3aed", color: "#fff", border: "none", borderRadius: "10px", fontWeight: 600, fontSize: "14px", cursor: "pointer" }}>{loading ? "Signing in..." : "Sign In as Admin"}</button>
        </form>
        <p style={{ color: "var(--color-text-muted, #737373)", fontSize: "12px", textAlign: "center", marginTop: "16px" }}>
          Bar manager? <a href="/bar/login" style={{ color: "#7c3aed", textDecoration: "none" }}>Sign in here</a>
        </p>
      </div>
    </div>
  );
